...
argument-hint: "[zgłoszenie klienta + ścieżki do załączników]"
---
...
- **`CLAUDE.md`** (`## Project Structure`) — mapa modułów, by przypisać sprawy do modułów.
...
**Czytaj załączniki jako dowód.** Read obsługuje obrazy (PNG/JPG) i pliki — otwórz każdy
załącznik przypisany do sprawy i wyciągnij konkret (kto, kiedy, jaka wartość, jaki ekran).
Nie opisuj załącznika ogólnikowo — wyciągnij fakt, który rozstrzyga sprawę.
...
1. **Znajdź flow.** Przejdź ścieżkę od zdarzenia do oczekiwanego efektu
   (np. korekta → akceptacja → trigger maila → wysyłka). Użyj Grep/Glob/Read.
...
- potwierdzony bug → wypisz jedną pełną komendę `debug` z `absolutpowers/problem/problem-{slug}.md "Sprawa N"` w składni aktywnego harnessu (debug wczyta raport i skupi się na nazwanej sprawie); stosuj `references/harness-command-contract.md`
- gap featurowy → wypisz jedną pełną komendę `feature-discuss` z argumentem `"{opis brakującej funkcji}"` w składni aktywnego harnessu; stosuj `references/harness-command-contract.md`
