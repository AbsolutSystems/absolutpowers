...
allowed-tools: Read, Glob, Grep, Bash(git:*), Bash(npx @mermaid-js/mermaid-cli:*), Write(**/docs/modules/**/*.md), Write(**/docs/architecture/**/*.html)
argument-hint: "[nazwa modułu lub ścieżka, np. auth lub src/billing]"
---
...
- ≠ `update-ai-context` — tamten skanuje CAŁE repo → broad/shallow CLAUDE.md. Tu: deep, jeden moduł, z diagramami.
...
- `CLAUDE.md` → `## Project Structure` (mapowanie katalogów na moduły/odpowiedzialności).
