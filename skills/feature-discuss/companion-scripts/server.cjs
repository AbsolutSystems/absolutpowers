...
// AbsolutPowers vendoring note: upstream's remote brand-image fetch has been
// removed entirely (constant + <img> branch deleted), not just env-gated.
// Telemetry is hardcoded off — no external GET is ever possible from this
// file, regardless of environment variables. See VENDORED.md.
...
function brandMarkup() {
  // Telemetry hardcoded off (see SUPERPOWERS_TELEMETRY_DISABLED above): no
  // remote logo image is ever rendered, only a local text label.
  const version = escapeHtmlText(SUPERPOWERS_VERSION);
  const text = 'AbsolutPowers (vendored companion) v' + version;
