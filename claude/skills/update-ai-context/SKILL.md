...
description: >
  Creates or updates hierarchical CLAUDE.md source files and mirrored AGENTS.md
  files, discovers and saves project patterns to ./absolutpowers/patterns.md, and
  proposes or audits project rules in ./absolutpowers/rules.md. Auto-detects
  bootstrap vs update mode.
...
1. **CLAUDE.md files** — hierarchical source-of-truth documentation for the project
2. **AGENTS.md files** — generated mirrors of sibling CLAUDE.md files for Codex
3. **`./absolutpowers/patterns.md`** — discovered code patterns and conventions
4. **`./absolutpowers/rules.md`** — project rules for code review compliance
...
Create `./absolutpowers/` directory if it doesn't exist.

---

## PHASE 1: CLAUDE.md Source Files (Bootstrap or Update)
...
This project uses hierarchical AI context shared across Claude Code and Codex.
- `CLAUDE.md`: editable source of truth for this directory scope
- `AGENTS.md`: generated mirror for Codex in the same directory scope
- `./absolutpowers/patterns.md`: discovered code patterns
- `./absolutpowers/rules.md`: project rules for code review

Run the `update-ai-context` skill to refresh documentation and sync mirrors.
```
...
7. After all CLAUDE.md edits, refresh sibling `AGENTS.md` mirrors

---

## PHASE 2: Patterns Discovery → `./absolutpowers/patterns.md`
...
### Format for `./absolutpowers/patterns.md`:

```markdown
# Project Patterns

> Auto-generated by `/absolutpowers:update-ai-context` on YYYY-MM-DD.
...
### Update mode
If `./absolutpowers/patterns.md` already exists:
...
## PHASE 3: Rules → `./absolutpowers/rules.md`

Rules are prescriptive constraints used by `/absolutpowers:review` during code review (Phase 3: Rules Check).

### Bootstrap (no `./absolutpowers/rules.md`)
...
> Used by `/absolutpowers:review` for automated rules checking.
...
### Update (`./absolutpowers/rules.md` exists)
...
After all `CLAUDE.md` edits are complete, create or update a sibling `AGENTS.md` for every `CLAUDE.md` in scope.

Rules for the mirror:
- `CLAUDE.md` is the editable source of truth
- `AGENTS.md` must live in the same directory as its sibling `CLAUDE.md`
- Keep scope identical: root-to-subtree and package-level mirrors stay aligned 1:1
- Mirror the content faithfully; only an autogenerated banner is allowed in `AGENTS.md`
- If a `CLAUDE.md` file is deleted because the package no longer exists, remove the stale sibling `AGENTS.md` too

If a bundled helper script is available, prefer using it to refresh the mirrors for the target project. Otherwise perform the mirroring directly.

---

## PHASE 5: Change Report

Output a summary of ALL changes across all areas:
...
### AGENTS.md Sync
- **Mirrored files updated:** [count]
- **Mirrored files created:** [count]
- **Mirrored files removed:** [count]

### Patterns (./absolutpowers/patterns.md)
- **New patterns found:** [count]
- **Removed (dead) patterns:** [count]
- **Updated references:** [count]

### Rules (./absolutpowers/rules.md)
...
6. Phase 4: Sync mirrored AGENTS.md files
7. Phase 5: Output change report
