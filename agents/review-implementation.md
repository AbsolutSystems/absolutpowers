...
- The same applies when your own re-run is killed by the shell tool's timeout before printing either `BUILD SUCCESSFUL` or `BUILD FAILED`: that is inconclusive, not a `[BLOCKER]` TESTS or COMPLETENESS finding. Narrow the scope and retry before concluding failure; if nothing finishes, report the check as inconclusive rather than as a failure the build never reported.
