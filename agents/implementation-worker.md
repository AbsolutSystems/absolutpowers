...
4. Before running, read `references/test-scope-policy.md` — it owns test scope and timing. Measure the change's blast radius before trusting any scope: if the radius is wider than the phase file's command covers, run the wider scope as well and say so under `Notes for orchestrator` — do not silently accept a command narrower than the change. Know this verification task's includes/excludes before relying on it, and report elapsed time beside the exit code: a fast green run may be a cache replay, not a real one. Run the phase verification commands. Do not pipe the output through `tail`, `head`, or `grep` — redirect to a file and read it if the output is long, or let it through unfiltered. Piping deletes gradle's `actionable tasks: X executed, Y up-to-date, Z from-cache` summary and the closing status line (`BUILD SUCCESSFUL in Xm Ys` or `BUILD FAILED in Xm Ys`), so a cache replay becomes indistinguishable from a real run.
   ...
   - When a hard timeout happens, first decide whether breadth was the point of this command: check this phase's change against `references/test-scope-policy.md` — the full-unit-suite row of its `What to run` table, plus the `Requires the full unit suite` closed list that names the cases the table row leaves implicit. Either one is enough to make breadth mandatory, and the blast radius you measured is what tells you whether this change is one of them.
     - If breadth is mandatory, narrowing is wrong — it would destroy the only reason the suite was run broad. Rerun the same broad target in the background.
     - If breadth is not mandatory, the timeout is incidental — narrow instead:
       ...
       c. If even the narrowest target will not fit, rerun that target in the background.
     - If neither path produces a result — the backgrounded broad run never finishes, or the backgrounded narrowest target never finishes — stop. Do not guess at pass or fail. Record the command as `timeout` in `Tests run` and name the exact target that would not complete under `Notes for orchestrator`.
   - Either path, a backgrounded run only counts if you actually read its completion output before writing any verdict — an unread background completion is the same gap wearing different clothes.
...
`pass/fail` in `Tests run` has a third value, `timeout`: the command was killed by the shell tool's hard limit before it produced a result (Process step 4). Use it exactly when that happened and neither a narrower rerun nor a backgrounded rerun finished either — never round a `timeout` to `pass`, and don't call it `fail` either, since the build never actually reported failure.
...
Return `BLOCKED` for hard failures, or a task that cannot be completed in its current shape (too large, or the plan itself is wrong) — and also when verification never produced a result: every narrower or backgrounded rerun from Process step 4 still hit the timeout. An unresolved `timeout` is not a confirmed failure, but it blocks the phase the same way one does, since the phase cannot be marked `completed` without a result.
